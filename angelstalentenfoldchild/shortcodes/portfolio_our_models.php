...
if ( !class_exists( 'avia_sc_portfolio_our_models' ) )
{
	class avia_sc_portfolio_our_models extends aviaShortcodeTemplate
	{
			/**
			...
			{
				$this->config['name']		= __('Portfolio Grid for Our-models', 'avia_framework' );
				$this->config['tab']		= __('Content Elements', 'avia_framework' );
				$this->config['icon']		= AviaBuilder::$path['imagesURL']."sc-portfolio.png";
				$this->config['order']		= 38;
				$this->config['target']		= 'avia-target-insert';
				$this->config['shortcode'] 	= 'av_portfolio_our_models';
			...
			function shortcode_handler($atts, $content = "", $shortcodename = "", $meta = "")
			{
				...
				$grid = new avia_post_grid_our_models($atts);
				$grid->query_entries();
...
if ( !class_exists( 'avia_post_grid_our_models' ) )
{
	class avia_post_grid_our_models
	{
		static  $grid = 0;
		...
		public function html()
		{
			...
			avia_post_grid_our_models::$grid ++;
			extract($this->atts);

			$container_id 		= avia_post_grid_our_models::$grid;
			$extraClass 		= 'first';
			$grid 				= 'one_fourth';
            ...
            //$angelstalent_dates = get_field('angelstalent_dates', $the_id);
                ...
                if($columns == "1" && $one_column_template == 'special')
                {
                    ...
                    if(!empty($image))
                    {
                        ...
                        $output .= "<".$link_markup[0]." data-rel='grid-".avia_post_grid_our_models::$grid."' class='grid-image avia-hover-fx 111'>".$custom_overlay.$image."</".$link_markup[1].">";
                        $output .= "</div>";
                    }
                ...
                else
                {
                    ...
                    $output .= "<article class='main_color inner-entry' ".avia_markup_helper(array('context' => 'entry','echo'=>false, 'id'=>$the_id, 'custom_markup'=>$custom_markup)).">";
                    $output .= apply_filters('avf_portfolio_extra', "", $entry);
                    $output .= "<".$link_markup[0]." data-rel='grid-".avia_post_grid_our_models::$grid."' class='grid-image avia-hover-fx 222'><div class='losange'><div class='los1'>".$custom_overlay.get_the_post_thumbnail( $the_id, $image_size, $image_attrs )."</div> </div></".$link_markup[1].">";
                    //$output .= $custom_overlay;
                    ...
                    $output .= "</article>";
                        $output .= '<div class="angel_info">';
                            $output .= '<p class="angel_info_title">'.$angelstalent_city.'</p>';
                            $output .= '<span class="angel_info_dates">'.$angelstalent_dates.'</span>';
                        $output .= '</div>';
                    $output .= "</div>";
                }
		...
		protected function sort_buttons($entries, $params)
		{
			...
			$output = "<div class='sort_width_container av-sort-".$this->atts['sort']."' data-portfolio-id='".avia_post_grid_our_models::$grid."' ><div id='js_sort_items' >";
		...
		{
			if(isset(avia_post_grid_our_models::$preview_template[$entry->ID])) return;
			avia_post_grid_our_models::$preview_template[$entry->ID] = true;
			...
			$output .= "<div class='ajax_slide ajax_slide_{$id}' data-slide-id='{$id}' >";
				...
				if(!empty($nogalleryclass)) $nogalleryclass .= ' first ';
                    ...
                    $output .= '<footer class="entry-footer">';
                        $output .= '<h3>'.$angelstalent_city.'</h3>';
                        $output .= '<span>'.$angelstalent_dates.'</span>';
                    $output .= '</footer>';
		...
		public function query_entries($params = array())
		{
			...
			$query = apply_filters('avia_post_grid_our_models_query', $query, $params);
...
add_filter('avia_post_grid_our_models_query','avia_order_by_random');
