...
if ( !class_exists( 'avia_sc_portfolio_custom' ) )
{
	class avia_sc_portfolio_custom extends aviaShortcodeTemplate
	{
			/**
			...
			{
				$this->config['name']		= __('Portfolio Grid Custom', 'avia_framework' );
				$this->config['tab']		= __('Content Elements', 'avia_framework' );
				$this->config['icon']		= AviaBuilder::$path['imagesURL']."sc-portfolio.png";
				$this->config['order']		= 38;
				$this->config['target']		= 'avia-target-insert';
				$this->config['shortcode'] 	= 'av_portfolio_custom';
			...
			function shortcode_handler($atts, $content = "", $shortcodename = "", $meta = "")
			{
				...
				$grid = new avia_post_grid_custom($atts);
				$grid->query_entries();
...
if ( !class_exists( 'avia_post_grid_custom' ) )
{
	class avia_post_grid_custom
	{
		static  $grid = 0;
		...
		public function html()
		{
			...
			avia_post_grid_custom::$grid ++;
			extract($this->atts);

			$container_id 		= avia_post_grid_custom::$grid;
			$extraClass 		= 'first';
			$grid 				= 'one_fourth';
            ...
            $angelstalent_dates = get_field('angelstalent_dates', $the_id);
                ...
                if($columns == "1" && $one_column_template == 'special')
                {
                    ...
                    if(!empty($image))
                    {
                        ...
                        $output .= "<".$link_markup[0]." data-rel='grid-".avia_post_grid_custom::$grid."' class='grid-image avia-hover-fx 111'>".$custom_overlay.$image."</".$link_markup[1].">";
                        $output .= "</div>";
                    }
                ...
                else
                {
                    ...
                    $output .= "<article class='main_color inner-entry' ".avia_markup_helper(array('context' => 'entry','echo'=>false, 'id'=>$the_id, 'custom_markup'=>$custom_markup)).">";
                    $output .= apply_filters('avf_portfolio_extra', "", $entry);
                    $output .= "<".$link_markup[0]." data-rel='grid-".avia_post_grid_custom::$grid."' class='grid-image avia-hover-fx 222'><div class='losange'><div class='los1'>".$custom_overlay.get_the_post_thumbnail( $the_id, $image_size, $image_attrs )."</div> </div></".$link_markup[1].">";
		...
		protected function sort_buttons($entries, $params)
		{
			...
			$output = "<div class='sort_width_container av-sort-".$this->atts['sort']."' data-portfolio-id='".avia_post_grid_custom::$grid."' ><div id='js_sort_items' >";
		...
		{
			if(isset(avia_post_grid_custom::$preview_template[$entry->ID])) return;
			avia_post_grid_custom::$preview_template[$entry->ID] = true;
		...
		public function query_entries($params = array())
		{
			...
			$query = apply_filters('avia_post_grid_custom_query', $query, $params);
...
add_filter('avia_post_grid_custom_query','avia_order_by_random');
